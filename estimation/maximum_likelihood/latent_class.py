...
from utils import safe_log, ZERO_LOWER_BOUND, ONE_UPPER_BOUND


class LatentClassFrankWolfeEstimator(Estimator):
    def can_estimate(self, model):
        return LatentClassModel == model.__class__
    ...
    def likelihood_loss_function_coefficients(self, transactions):
        ...
        return [(transaction, amount_of_sales) for transaction, amount_of_sales in sales_per_transaction.items()]

    def look_for_new_mnl_model(self, model, likelihood_loss_function_coefficients):
        possible_mnl_model = MultinomialLogitModel.default_naive(model.products)
    ...
    def estimate(self, model, transactions):
        ...
        for _ in range(max_iterations):
            ...
            if likelihood_does_not_increase or likelihood_does_not_increase_enough:
                break
