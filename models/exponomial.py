...
class ExponomialModel(Model):
    ...
    def g(self, product, offered_products):
        better_products = filter(lambda p: self.utility_for(p) >= self.utility_for(product), offered_products)
    ...
    def probability_of(self, transaction):
        ...
        worse_products = filter(lambda p: self.utility_for(p) < self.utility_for(transaction.product),
...
class ExponomialModelConstraints(Constraints):
    ...
    def lower_bounds_vector(self):
        return ones(len(self.model.parameters_vector())) * utils.NLP_LOWER_BOUND_INF
